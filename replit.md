...
Capture URL: `runPublishPipeline` (in `routes/deployments.ts`) screenshots the **preview URL** (`${SITE_URL}/api/preview/{u}/{s}/`), NOT the live Vercel URL. The preview URL renders the same files server-side via the in-builder iframe route — no auth, no cache. `disableProjectProtection` is now called for ALL user deployments (not just templates) so Vercel auth/SSO is cleared on every publish.
...
- Explore page (`explore.tsx`), landing page template cards (`landing.tsx`), the user dashboard (`dashboard.tsx`), and the public project landing page (`project.tsx`) all use `screenshotUrl ?? coverImageUrl` with `object-cover object-top` cropping to show above-the-fold content. Card thumbnails use `aspect-[16/10]` so the 1280×800 capture viewport fills the width without side-cropping.
- The public project page (`project.tsx`) shows the static screenshot/cover image in place of a live iframe whenever one is available; it falls back to the in-builder preview iframe (`/api/preview/{u}/{s}/`) only when neither image exists yet (e.g. brand-new, never-published projects). This avoids the JSX-via-Babel compile flash on every visit and removes the per-pageview load on the api-server's preview route.
