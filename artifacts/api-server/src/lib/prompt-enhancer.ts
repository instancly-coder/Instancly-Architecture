...
11. Re-read the system prompt's "Runtime safety" section before sending — especially: no \`fetch()\`/\`axios\`/\`XMLHttpRequest\`, no \`<script src="X">\` tags pointing at files you aren't emitting, no \`import\`/\`export\`, no TypeScript syntax. Forms use \`mailto:\` or local state, never a fake \`/api/...\` endpoint.
${verticalBlock}
